...
class AutCommandNode(Node):
    ...
    def __init__(self):
        ...
        self.position_sub_ = self.create_subscription(LinActPos,'compressed_pub/depth',self.take_distance,10)
        ...
        self.camera_switch_pub_ = self.create_publisher(Bool,'camera_active',10)
    ...
    def get_command(self,msg):
        ...
        if not(self.in_progress):
            ...
            if not(self.started):
                self.started = True
        if self.button[10] > 0 and not(self.camera_pressed):
            self.camera_pressed = True
            self.camera_switch = not self.camera_switch
            camera = Bool()
            camera.data = self.camera_switch
            self.camera_switch_pub_.publish(camera)
    ...
    def take_distance(self,msg):
        distance_1 = int(msg.rl_lin_act[3])
        distance_2 = int(msg.rl_lin_act[2])
        if distance_1 != 0:
            self.distances[0] = distance_1
        if distance_2 != 0:
            self.distances[1] = distance_2
        self.get_logger().info(str(self.distances))
    ...
    def main_algorithm(self):
        ...
        if self.command == "stop":
            ...
            if not(self.in_progress):
                self.counter = 0
                linact = self.angle2inch([0.300,0.898])
            ...
            target_pos = 400
            diff = self.distances[0]-target_pos
            ...
            target_pos = 3000
            diff = self.distances[1]-target_pos
            request_motor = MotorsVel()
            if diff > 0:
                request_motor.rl_motor = [-0.75,0.75]
                self.motor_publisher_.publish(request_motor)
            else:
                request_motor.rl_motor = [0.0,0.0]
                self.motor_publisher_.publish(request_motor)
